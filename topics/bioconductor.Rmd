...
```{r}
myrange <- GRanges("chrI", IRanges(start=3,end=5), strand="+")
myrange
...
We mention Seqinfo objects mostly because they are something you may trip over if they are wrong.
...
This has been a whirlwind tour of one set of core classes in Bioconductor. The main thing you should take away from this is how to examine Bioconductor objects where to look for documentation. There are many many Bioconductor packages for working with specific types of data, which we hope will be accessible now that you have a general sense of how things work.
